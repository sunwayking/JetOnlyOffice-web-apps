...
const mobileRuntimeWorkflow = await readFile(
    new URL('.github/workflows/mobile-runtime.yml', repositoryRoot),
    'utf8',
);
...
test('PDF Desktop gap audit pins its source commit and direct-assignment scope', async () => {
    ...
    assert.match(
        mobileRuntimeWorkflow,
        /uses:\s*actions\/checkout@v4[\s\S]*?fetch-depth:\s*0/,
        'locked source commit verification requires a full-history checkout',
    );
